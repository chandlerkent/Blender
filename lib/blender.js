...
OptionsParser.usage("INPUT_FILE [OUTPUT_FILE]");
...
exports.main = function() {
    ...
    var output = (options.args[1] ? File.dirname(input) + "/" + options.args[1] : input);
    if (File.extension(output) !== "")
        output = output.replace(File.extension(output), "");
    output += ".j";
    ...
    stylesheet.generate(tw, options.args);
